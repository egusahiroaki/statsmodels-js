...
class Vector {
  ...
  skewness() {
    ...
    if (Object.is(a * _.ceil(b, 10), -0) || Object.is(a * _.ceil(b, 10), +0)) {
      return 0;
    }
    // for floating point number
    return a * _.ceil(b, 10);
  }
  ...
  kurtosis() {
    console.log("kurtosis()kurtosis()kurtosis()");
    const n = this.length();
    if (n <= 3) {
    ...
    let b = 0;
    // console.log("分母: " + Math.pow(Math.sqrt(this.unbiasedVar()), 4));
    for (let i = 0; i < n; i++) {
      // console.log(Math.pow(this._array[i] - this.mean(), 4));
      b +=
